...
int main(int argc, char **argv)
{
    ...
    /* 6th Test */
    page_table_update(pt, 0b1111111111, 0x1212);
    assert(page_table_query(pt, 0b1111111111) == 0x1212);
    tmp = phys_to_virt(pt << 13);
    tmp = phys_to_virt((tmp[0] >> 13) << 13);
    tmp = phys_to_virt((tmp[0] >> 13) << 13);
    tmp = phys_to_virt((tmp[0] >> 13) << 13);
    tmp = phys_to_virt((tmp[0] >> 13) << 13);
    tmp[1023] = ((tmp[1023] >> 1) << 1);

    assert(page_table_query(pt, 0b1111111111) == NO_MAPPING);
